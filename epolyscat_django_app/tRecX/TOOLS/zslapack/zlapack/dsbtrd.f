...
*>
*  =====================================================================
      SUBROUTINE zSBTRD( VECT, UPLO, N, KD, AB, LDAB, D, E, Q, LDQ,
     $                   WORK, INFO )
      implicit none
*
...
*
      IF( INITQ )
     $   CALL zslaSET( 'Full', N, N, ZERO, ONE, Q, LDQ )
*
...
*
                     CALL zslaRGV( NR, AB( 1, J1-1 ), INCA, WORK( J1 ),
     $                            KD1, D( J1 ), KD1 )
*
...
*
                     IF( NR.GE.2*KD-1 ) THEN
                        DO 10 L = 1, KD - 1
                           CALL zslaRTV( NR, AB( L+1, J1-1 ), INCA,
     $                                  AB( L, J1 ), INCA, D( J1 ),
     $                                  WORK( J1 ), KD1 )
   10                   CONTINUE
*
...
*
                        CALL zsrot( K-3, AB( KD-K+4, I+K-2 ), 1,
...
*
                  IF( NR.GT.0 )
     $               CALL zslaR2V( NR, AB( KD1, J1-1 ), AB( KD1, J1 ),
     $                            AB( KD, J1 ), INCA, D( J1 ),
     $                            WORK( J1 ), KD1 )
*
...
*
                        DO 30 L = 1, KD - 1
                           ...
                           END IF
                           IF( NRT.GT.0 )
     $                        CALL zslaRTV( NRT, AB( KD-L, J1+L ), INCA,
     $                                     AB( KD-L+1, J1+L ), INCA,
     $                                     D( J1 ), WORK( J1 ), KD1 )
   30                   CONTINUE
                     ELSE
                        ...
                        IF( J1END.GE.J1 ) THEN
                           DO 40 JIN = J1, J1END, KD1
                              CALL zsrot( KD-1, AB( KD-1, JIN+1 ), INCX,
     $                                   AB( KD, JIN+1 ), INCX,
     $                                   D( JIN ), WORK( JIN ) )
   40                      CONTINUE
                        END IF
                        ...
                        LAST = J1END + KD1
                        IF( LEND.GT.0 )
     $                     CALL zsrot( LEND, AB( KD-1, LAST+1 ), INCX,
...
*
     ...
     $                     IQAEND = IQAEND + KD
                        ...
                        DO 50 J = J1, J2, KD1
                           ...
                           CALL zsrot(NQ, Q( IQB, J-1 ), 1, Q( IQB, J ),
     $                                1, D( J ), WORK( J ) )
   50                   CONTINUE
                     ELSE
*
                        DO 60 J = J1, J2, KD1
                           CALL zsrot( N, Q( 1, J-1 ), 1, Q( 1, J ), 1,
     $                                D( J ), WORK( J ) )
   60                   CONTINUE
                     END IF
*
...
*                    elements which have been created outside the band
*
                     CALL zslaRGV( NR, AB( KD1, J1-KD1 ), INCA,
...
*
                     IF( NR.GT.2*KD-1 ) THEN
                        DO 130 L = 1, KD - 1
                           CALL zslaRTV(NR, AB( KD1-L, J1-KD1+L ), INCA,
  ...
  130                   CONTINUE
                     ELSE
                        ...
                        DO 140 JINC = J1, JEND, KD1
                           CALL zsrot( KDM1, AB( KD, JINC-KD ), INCX,
     $                                AB( KD1, JINC-KD ), INCX,
     $                                D( JINC ), WORK( JINC ) )
  140                   CONTINUE
                     END IF
...
*                       within the band
*
                        CALL zslaRTG( AB( K-1, I ), AB( K, I ),
     $                               D( I+K-1 ), WORK( I+K-1 ), TEMP )
                        AB( K-1, I ) = TEMP
*
*                       apply rotation from the left
*
                        CALL zsrot( K-3, AB( K-2, I+1 ), LDAB-1,
...
*
                  IF( NR.GT.0 )
     $               CALL zslaR2V( NR, AB( 1, J1-1 ), AB( 1, J1 ),
     $                            AB( 2, J1-1 ), INCA, D( J1 ),
     $                            WORK( J1 ), KD1 )
*
                  ...
                  IF( NR.GT.0 ) THEN
                     IF( NR.GT.2*KD-1 ) THEN
                        DO 150 L = 1, KD - 1
                           ...
                           END IF
                           IF( NRT.GT.0 )
     $                        CALL zslaRTV( NRT, AB( L+2, J1-1 ), INCA,
     $                                     AB( L+1, J1 ), INCA, D( J1 ),
     $                                     WORK( J1 ), KD1 )
  150                   CONTINUE
                     ELSE
                        ...
                        IF( J1END.GE.J1 ) THEN
                           DO 160 J1INC = J1, J1END, KD1
                              CALL zsROT( KDM1, AB( 3, J1INC-1 ), 1,
