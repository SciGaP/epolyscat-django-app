...
*
*  =====================================================================
      SUBROUTINE zsLASCL( TYPE, KL, KU, CFROM, CTO, M, N, A, LDA, INFO )
*
...
c$$$      DOUBLE PRECISION   BIGNUM, CFROM1, CFROMC, CTO1, CTOC, MUL, SMLNUM
      real(8)      BIGNUM, CFROM1, CFROMC, CTO1, CTOC, SMLNUM
      complex(8)    MUL
*     ..
*     .. External Functions ..
...
*
      IF( ITYPE.EQ.-1 ) THEN
         INFO = -1
      ELSE IF( abs(CFROM).EQ.ZERO .OR. DISNAN(dble(CFROM)) ) THEN
         INFO = -4
      ELSE IF( DISNAN(dble(CTO)) ) THEN
         INFO = -5
      ELSE IF( M.LT.0 ) THEN
         INFO = -6
...
!           serves as the correct multiplication factor.
            ...
            DONE = .TRUE.
            CFROMC = ONE
         ELSE IF( ABS(CFROM1).GT.ABS(CTOC).AND.abs(CTOC).NE.ZERO)THEN
            MUL = SMLNUM
            DONE = .FALSE.
            CFROMC = CFROM1
