...
*
*  =====================================================================
      SUBROUTINE zSBGST( VECT, UPLO, N, KA, KB, AB, LDAB, BB, LDBB, X,
     $                   LDX, WORK, INFO )
*
...
*     ..
*     .. Array Arguments ..
c$$$      DOUBLE PRECISION   AB( LDAB, * ), BB( LDBB, * ), WORK( * ),
c$$$     $                   X( LDX, * )
      complex(8)   AB( LDAB, * ), BB( LDBB, * ), WORK( * ),
     $                   X( LDX, * )
*     ..
*
*  =====================================================================
...
c$$$      DOUBLE PRECISION   ZERO, ONE
      complex(8)   ZERO, ONE
      PARAMETER          ( ZERO = (0.0D+0,0.d0), ONE = (1.0D+0,0.d0) )
*     ..
*     .. Local Scalars ..
      LOGICAL            UPDATE, UPPER, WANTX
      INTEGER            I, I0, I1, I2, INCA, J, J1, J1T, J2, J2T, K,
     $                   KA1, KB1, KBT, L, M, NR, NRT, NX
c$$$      DOUBLE PRECISION   BII, RA, RA1, T
      complex(8)   BII, RA, RA1, T
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      EXTERNAL           LSAME
...
*     .. External Subroutines ..
      EXTERNAL           zsger, zslar2V, zslargv, zslartg, zslarTV, zslaset,
     $                   zsrot, zSCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      IF( WANTX )
     $   CALL zslaset( 'Full', N, N, ZERO, ONE, X, LDX )
*
...
*
               CALL zSCAL( N-M, ONE / BII, X( M+1, I ), 1 )
               IF( KBT.GT.0 )
     $            CALL zsger( N-M, KBT, -ONE, X( M+1, I ), 1,
...
*
                  CALL zslartg( AB( K+1, I-K+KA ), RA1,
...
*
            IF( NRT.GT.0 )
     $         CALL zslargv(NRT, AB( 1, J2T ), INCA, WORK( J2T-M ), KA1,
     $                      WORK( N+J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 100 L = 1, KA - 1
                  CALL zslarTV( NR, AB( KA1-L, J2 ), INCA,
...
*              blocks
*
               CALL zslar2V( NR, AB( KA1, J2 ), AB( KA1, J2+1 ),
...
*
            DO 110 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( L, J2+KA1-L ), INCA,
...
*
               DO 120 J = J2, J1, KA1
                  CALL zsrot( N-M, X( M+1, J ), 1, X( M+1, J+1 ), 1,
     $                       WORK( N+J-M ), WORK( J-M ) )
  120          CONTINUE
            END IF
  130    CONTINUE
...
*
            DO 140 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( L, J2-L+1 ), INCA,
...
*
               CALL zslargv( NR, AB( 1, J2 ), INCA, WORK( J2 ), KA1,
     $                      WORK( N+J2 ), KA1 )
*
...
*
                  CALL zslartg( AB( KA1-K, I ), RA1, WORK( N+I-K+KA-M ),
     $                         WORK( I-K+KA-M ), RA )
*
...
*
            IF( NRT.GT.0 )
     $       CALL zslargv( NRT, AB( KA1, J2T-KA ), INCA, WORK( J2T-M ),
...
*
               DO 330 L = 1, KA - 1
                  CALL zslarTV( NR, AB( L+1, J2-L ), INCA,
...
*              blocks
*
               CALL zslar2V( NR, AB( 1, J2 ), AB( 1, J2+1 ),AB( 2, J2 ),
...
*
            DO 340 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( KA1-L+1, J2 ), INCA,
...
*
            DO 370 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( KA1-L+1, J2-KA ), INCA,
...
*              which have been created outside the band
*
               CALL zslargv(NR, AB( KA1, J2-KA ), INCA, WORK( J2 ), KA1,
     $                      WORK( N+J2 ), KA1 )
*
...
*              post-multiply X by inv(S(i))
*
               CALL zSCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )
     $         CALL zsger( NX, KBT, -ONE, X( 1, I ), 1, BB( KB, I+1 ),
     $                       LDBB-1, X( 1, I+1 ), LDX )
            END IF
*
*           store a(i1,i) in RA1 for use in next loop over K
...
*                 generate rotation to annihilate a(i+k-ka-1,i)
*
                  CALL zslartg( AB( K+1, I ), RA1, WORK( N+I+K-KA ),
     $                         WORK( I+K-KA ), RA )
*
*                 create nonzero element a(i+k-ka-1,i+k) outside the
...
*
            IF( NRT.GT.0 )
     $         CALL zslargv( NRT, AB( 1, J1+KA ), INCA, WORK( J1 ), KA1,
     $                      WORK( N+J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
*              apply rotations in 1st set from the left
*
               DO 580 L = 1, KA - 1
                  CALL zslarTV( NR, AB( KA1-L, J1+L ), INCA,
...
*              blocks
*
               CALL zslar2V( NR, AB( KA1, J1 ), AB( KA1, J1-1 ),
...
*
            DO 590 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( L, J1T ), INCA,
...
*
               DO 600 J = J1, J2, KA1
                  CALL zsrot( NX, X( 1, J ), 1, X( 1, J-1 ), 1,
     $                       WORK( N+J ), WORK( J ) )
  600          CONTINUE
            END IF
  610    CONTINUE
...
*
            DO 620 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( L, J1T+KA ), INCA,
...
*              which have been created outside the band
*
               CALL zslargv( NR, AB( 1, J1+KA ), INCA, WORK( M-KB+J1 ),
...
*
               CALL zSCAL( NX, ONE / BII, X( 1, I ), 1 )
               IF( KBT.GT.0 )
     $           CALL zsger( NX, KBT, -ONE, X( 1, I ), 1, BB( 2, I ), 1,
     $                       X( 1, I+1 ), LDX )
            END IF
*
*           store a(i,i1) in RA1 for use in next loop over K
...
*                 generate rotation to annihilate a(i,i+k-ka-1)
*
                  CALL zslartg( AB( KA1-K, I+K-KA ), RA1,
...
*
            IF( NRT.GT.0 )
     $         CALL zslargv( NRT, AB( KA1, J1 ), INCA, WORK( J1 ), KA1,
     $                      WORK( N+J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
*              apply rotations in 1st set from the right
*
               DO 810 L = 1, KA - 1
                  CALL zslarTV(NR, AB( L+1, J1 ), INCA, AB( L+2, J1-1 ),
...
*              blocks
*
               CALL zslar2V( NR, AB( 1, J1 ), AB( 1, J1-1 ),
...
*
            DO 820 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( KA1-L+1, J1T-KA1+L ), INCA,
...
*
            DO 850 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL zslarTV( NRT, AB( KA1-L+1, J1T+L-1 ), INCA,
...
*              which have been created outside the band
*
               CALL zslargv( NR, AB( KA1, J1 ), INCA, WORK( M-KB+J1 ),
     $                      KA1, WORK( N+M-KB+J1 ), KA1 )
*
*              apply rotations in 2nd set from the right
*
