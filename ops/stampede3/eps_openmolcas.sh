...
if [[ -d "$scratch_candidate" ]]; then
    ...
    mkdir -p "$MOLCAS_WORKDIR" || fail "cannot create work directory: $MOLCAS_WORKDIR" 73
fi
...
molcas_root=${MOLCAS_ROOT:-/home1/00421/ccguser/apps/molcas}
pymolcas=${PYMOLCAS:-$molcas_root/pymolcas}
if [[ -z "${PYMOLCAS:-}" ]] && type module >/dev/null 2>&1; then
    module load python >/dev/null 2>&1 || true
fi
[[ -x "$pymolcas" ]] || fail "pymolcas is missing or not executable: $pymolcas" 69

output="${input}.out"
"$pymolcas" "$input" | tee "$output"
molcas_status=${PIPESTATUS[0]}
if [[ "$molcas_status" -ne 0 ]]; then
    fail "pymolcas exited with status $molcas_status; inspect $output for details" "$molcas_status"
fi

if [[ -f molcas.status ]] && ! grep -q "Happy landing" molcas.status; then
...
project=${project%.*}
molden_file=""
if [[ -f "$project.rasscf.molden" ]]; then
    molden_file="$project.rasscf.molden"
elif [[ -f "$project.scf.molden" ]]; then
    molden_file="$project.scf.molden"
fi

render_orbitals=${3:-yes}
if [[ "$render_orbitals" != "no" && -n "$molden_file" ]]; then
