...
[[ -r "$molden_file" ]] ||
    fail "input file is missing or unreadable: $molden_file" 66
mkdir -p "$output_dir" ||
    fail "cannot create output directory: $output_dir" 73
...
jmol_jar=${JMOL_JAR:-/home/ampuser/apps/xchem/jmol-14.31.34/JmolData.jar}
jmol_template=${JMOL_TEMPLATE:-$script_dir/print_frontier_orbitals.template}
...
for view in "${views[@]}"; do
    ...
    if ! "$java_cmd" -jar "$jmol_jar" -g800x600 -ions "$jmol_script" \
        > "$jmol_log" 2>&1; then
