<template>
    <b-card>
        <div class="d-flex flex-row">
            <div>
                <LoadingOverlay name="plot" class="h-100">
                    <img v-if="plotImageUrl != null" class="w-100" :src="plotImageUrl" 
                    :style="{opacity: (imageSrcFilename == file.name) ? '100%' : '30%'}"/>
                    <img v-else class="w-100" :src="defaultImage" style="opacity: 30%"/>
                </LoadingOverlay>
            </div>
            <div class="d-flex flex-column">
                ...
                <div class="d-flex flex-row justify-content-between w-100">
                    <b-form-group label="x-axis" v-slot="{ ariaDescribedby }" class="mx-1" :disabled="plotParameter != 'new'">
                        <b-form-radio-group
                            id="radio-group-1"
                            v-model="xAxisIndex"
                    ...
                    <b-form-group label="y-axes" v-slot="{ ariaDescribedby }" class="mx-1" :disabled="plotParameter != 'new'">
            ...
            <b-form-group v-if="plotableFiles.length > 1" label="Plotted Files" v-slot="{ ariaDescribedby }" class="mx-3">
                <b-form-checkbox-group
                    id="plotted-files-checkboxes"
                    v-model="plottedFiles"
                    :options="plotableFiles"
                    :aria-describedby="ariaDescribedby"
                    name="plotted-files"
                    stacked
                ></b-form-checkbox-group>
...
export default {
    ...
    computed: {
        ...
        xAxisOptions() {
            const { plotObject } = this.$store.getters["input/getSelectedInfo"]();

            if (plotObject != null)
                return plotObject.xOptions;
            else 
                return []
        },
        yAxisOptions() {
            ...
            if (plotObject != null)
                return plotObject.yOptions;
            else 
                return []
        },
        plotableFiles() {
            const { properFilename, selected } = this.$store.getters["input/getSelectedInfo"]();

            return this.$store.getters["input/getFilesInCategory"](properFilename)
                .map((file) => ({ 
                    text: file.name, 
                    value: file["data-product-uri"],
                    disabled: file.name == selected
                }));
        },
        canCreate() {
            return this.yAxisIndeces.length > 0 && this.yAxisIndeces.indexOf(this.xAxisIndex) == -1
        }
    },
    methods: {
            ...
            try {
                this.plotParameters = (await this.$store.dispatch("plotParameters/fetchPlotParameters"))
                    .map(({xAxis, yAxes, flags}) => {
                        const obj = plotObject.xOptions.find(({ value }) => value == xAxis);
                        const xAxisName = (obj != null && "text" in obj) ? obj.text : "--";

                        const yAxisNames = yAxes.split(",").map(yAxis => {
                            const obj = plotObject.yOptions.find(({ value }) => value == parseInt(yAxis));
        ...
        async fetchPlot() {
            ...
            // {runIds, plotfiles, xAxis, yAxis, flags}
            PlotService.plotSelectedRuns({
                runIds: [this.runId],
                plotfiles: this.plottedFiles.map(plottedFile => ({
                    dataProductURI: plottedFile,
                    prefix: ""
                })),
    ...
    watch: { 
        ...
        file() {
            if (this.plottedFiles.indexOf(this.file["data-product-uri"]) == -1) {
                this.plottedFiles = [this.file["data-product-uri"]];
            } else {
                const index = this.plottedFiles.indexOf(this.file["data-product-uri"]);
                this.plottedFiles = [
                    this.file["data-product-uri"], 
                    ...this.plottedFiles.slice(0, index), 
                    ...this.plottedFiles.slice(index + 1)
                ];
            }
        }
    },
